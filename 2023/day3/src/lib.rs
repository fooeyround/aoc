...
fn part_one(input: &Vec<String>) {
    ...
    let symbols = schematic.map.iter().filter_map(|entry| {
        match entry {
    ...
    let parts = schematic.map.iter().filter_map(|entry| {
        match entry {
    ...
    let touching_parts = parts.iter().filter( | part | {
        symbols.iter().any(|symbol| {
            is_part_touching_pos(part, &symbol.position)
        })
    }).collect::<Vec<&&Part>>();


    let sum: u64 = touching_parts.iter().fold(0, |acc, part| {
        acc + part.value as u64
    });
...
fn part_two(input: &Vec<String>) {
    ...
    let symbols_touching_exactly_two_parts = symbols.iter().filter(| s | s.symbol == '*').filter_map(|symbol| {
        let connected_part_values= parts.iter().filter_map(|part| {
    ...
    let sum: u64 = symbols_touching_exactly_two_parts.iter().map(|values| {
        values.0 as u64 * values.1 as u64
    }).sum();
    ...
    for (y, line) in input.iter().enumerate() {
        ...
        for (x, c) in line.chars().enumerate() {
            ...
            if let Some(_) = c.to_digit(10) {
                ...
                let folded_value_string = iter.iter().fold(String::new(), |mut acc, c| { acc.push(*c); acc });
                ...
                map.push(EntryType::PART(Part { positions, value: parsed_value }));
...
fn is_part_touching_pos(part: &Part, position: &Position) -> bool {
    part.positions.iter().any(| part_pos| {
        distance_between(part_pos.x, position.x) <= 1 && distance_between(position.y, part_pos.y) <= 1
    })
}
